...
namespace DVotingBackendApp.services
{
    public class VoterService : IVoterService
    {
        ...
        public async Task<Dictionary<string, List<string>>> FetchIrisSampleAsync(string constituency)
        {
            ...
            Dictionary<string, List<string>> irisSamplesMap = new Dictionary<string, List<string>>();

            var voterTasks = response.Select(async id =>
            {
                var irisSample = await _voterRepository.FetchIrisSampleAsync(id);

                irisSamplesMap[id] = irisSample;
            });

            await Task.WhenAll(voterTasks);


            return irisSamplesMap;
        }
